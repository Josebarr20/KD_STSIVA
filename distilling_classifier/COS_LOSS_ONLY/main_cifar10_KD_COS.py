...
def main(args):
  ...
  wandb.init(project=args.project_name, name="KD_cos_" + path_name, config=args)
  ...
  for epoch in range(args.num_epochs):
    ...
    for _, train_data in data_loop_train:
      ...
      target = torch.ones(HTs.size(0)).to(device)
      loss_cos = COS_LOSS(HTs.reshape(HTt.size(0), -1), HTt.reshape(HTt.size(0), -1), target)
      loss_train = (args.lambda1*loss_deco + args.lambda2*loss_cos)
      ...
      train_cos_loss.update(loss_cos.item())
    ...
    with torch.no_grad():
      ...
      for _, val_data in data_loop_val:
        ...
        target = torch.ones(HTs.size(0)).to(device)
        loss_cos = COS_LOSS(HTs.reshape(HTt.size(0), -1), HTt.reshape(HTt.size(0), -1), target)
        loss_val = (args.lambda1*loss_deco + args.lambda2*loss_cos)

        val_loss.update(loss_val.item())
        val_deco_loss.update(loss_deco.item())
        val_cos_loss.update(loss_cos.item())
    ...
    wandb.log({"train_loss": train_loss.avg,
                ...
                "val_acc": val_acc.avg,
                "train_cos_loss": train_cos_loss.avg,
                "train_deco_loss": train_deco_loss.avg,
                "val_cos_loss": val_cos_loss.avg,
  ...
  with torch.no_grad():
    ...
    for _, test_data in data_loop_test:
      ...
      loss_test = (args.lambda1*loss_deco + args.lambda2*loss_cos)
      test_loss.update(loss_test.item())
      test_deco_loss.update(loss_deco.item())
      test_cos_loss.update(loss_cos.item())
  ...
  wandb.log({"test_loss": test_loss.avg,
              ...
              "test_cos_loss": test_cos_loss.avg})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--SPC_portion_tchr", type=float, default=0.4)
    parser.add_argument("--SPC_portion_st", type=float, default=0.2)
    ...
    parser.add_argument(
        "--teacher_path",
        type=str,
        default=r"save_model_t\model_binary_40.pth",
    )
    parser.add_argument("--save_path", type=str, default="WEIGHTS/SPC_KD_TEST/")
    parser.add_argument("--project_name", type=str, default="KD_COS_TEST")
